...
function ResumeCard({ resume, onEdit, onDelete, onDuplicate, onDownload }) {
...
function ResumeThumb({ theme, data }) {
  const name = data?.personal?.fullName || 'Your Name'
  const title = data?.personal?.jobTitle || 'Job Title'
...
export default function DashboardPage() {
  const { user, profile, resumes, resumesLoading, fetchResumes, createResume, deleteResume, duplicateResume } = useStore()
  ...
  const handleCreate = async (initialData = null) => {
    try {
            ...
            <div className={styles.planCardProgress}>
              ...
              <div className={styles.planCardProgressBar}>
                <div
                  ...
                  style={{ width: `${Math.min((resumeCount / Math.min(resumeLimit, 25)) * 100, 100)}%` }}
                />
              </div>
            </div>
            {profile?.plan === 'free' && (
