...
export async function enhanceTextWithAI(text) {
    ...
    if (!apiKey) {
        // Check if user is using the mock mode
...
export async function parseResumeWithAI(pdfText) {
    ...
    if (!apiKey) {
        ...
        return {
            personal: { fullName: "Imported Name", jobTitle: "Imported Role", email: "imported@email.com", phone: "555-1234", location: "Remote", website: "" },
            summary: "Imported Summary: " + pdfText.substring(0, 100),
            experience: [{ id: Date.now(), title: "Imported Job", company: "Company", startDate: "2020", endDate: "2024", current: false, description: "Imported description" }],
            education: [],
...
export async function importFromLinkedInProfile(url) {
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    const fallbackData = {
        personal: { fullName: "John Doe", jobTitle: "Software Engineer", email: "john@example.com", phone: "555-0123", location: "Remote", website: url },
...

{
  ...
  "skills": ["skill1", "skill2"]
        ...
        return fallbackData; // Fallback safely
    }
}
